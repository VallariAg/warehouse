...
export default function Card({board, username}) {
    const link_count = board.link_data_aggregate.aggregate.count;
    const board_url = `/${username}/${board.board_name.replace(" ", "-")}`;
    
    return (
        <Link to={board_url} className="mx-1 my-0 bg-gray-50 p-1 rounded-lg">
